...
<el-col :span="4">
    ...
    <el-table-column
      ...
      <template slot-scope="scope">
        <el-button type="text" size="small" @click="watchDetail(scope.row)">查看</el-button>
...
export default {
  ...
  data() {
    return {
      currentContentNName: '人力资源档案查询', // 通过路由传参数实现
      options: [{
        value: '选项1',
        label: '黄金糕'
      }, {
  ...
  methods: {
    ...
    watchDetail(row) {
      console.log(row);
      // 跳转到审核页面，传递详情id
      this.$router.push({name: 'watchOrEditFile', params: {id: row.id}});
    },
    deleteRow(row) {
      console.log(0);
